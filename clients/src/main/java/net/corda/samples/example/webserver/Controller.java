...
import java.util.*;
import net.corda.samples.example.flows.SendMessage;
import net.corda.samples.example.flows.Payment;
import net.corda.samples.example.flows.Verification;
...
public class Controller {
    ...
    public List<StateAndRef<PaymentState>> getIOUs() {
        ...
        return proxy.vaultQuery(PaymentState.class).getStates();
    }

    @PostMapping(value = "create-iou", produces = TEXT_PLAIN_VALUE, headers = "Content-Type=application/x-www-form-urlencoded")
    public ResponseEntity<String> issueMessage(HttpServletRequest request) throws IllegalArgumentException {
        ...
        try {
            ...
            SignedTransaction result = proxy.startTrackedFlowDynamic(Payment.InitiatePayment.class, amount, otherParty).getReturnValue().get();
            // Return the response.
    ...
    @PostMapping(value = "create-msg", produces = TEXT_PLAIN_VALUE, headers = "Content-Type=application/x-www-form-urlencoded")
    public ResponseEntity<String> issueIOU(HttpServletRequest request) throws IllegalArgumentException {
        ...
        CordaX500Name partyX500Name = CordaX500Name.parse(party);
        Party otherParty1 = proxy.wellKnownPartyFromX500Name(partyX500Name);

        // Create a new IOU state using the parameters given.
        try {
            // Start the IOUIssueFlow. We block and waits for the flow to return.
            SignedTransaction result = proxy.startTrackedFlowDynamic(SendMessage.InitiatorM.class, msg, otherParty1).getReturnValue().get();
            // Return the response.
            return ResponseEntity
                    .status(HttpStatus.CREATED)
                    .body("Transaction id " + result.getId() + " committed to ledger.\n " + result.getTx().getOutput(0));
    ...
    @PostMapping(value = "create-answer", produces = TEXT_PLAIN_VALUE, headers = "Content-Type=application/x-www-form-urlencoded")
    public ResponseEntity<String> issueAnswer(HttpServletRequest request) throws IllegalArgumentException {
        ...
        CordaX500Name partyX500Name = CordaX500Name.parse(party);
        Party otherParty2 = proxy.wellKnownPartyFromX500Name(partyX500Name);

        // Create a new IOU state using the parameters given.
        try {
            // Start the IOUIssueFlow. We block and waits for the flow to return.
            SignedTransaction result = proxy.startTrackedFlowDynamic(Verification.InitiatorV.class, answer, percentage, otherParty2).getReturnValue().get();
            // Return the response.
            return ResponseEntity
                    .status(HttpStatus.CREATED)
                    .body("Transaction id " + result.getId() + " committed to ledger.\n " + result.getTx().getOutput(0));
    ...
    public ResponseEntity<List<StateAndRef<PaymentState>>> getMyIOUs() {
        List<StateAndRef<PaymentState>> myious = proxy.vaultQuery(PaymentState.class).getStates().stream().filter(
                it -> it.getState().getData().getSender().equals(proxy.nodeInfo().getLegalIdentities().get(0))).collect(Collectors.toList());
        return ResponseEntity.ok(myious);
    }

    @GetMapping(value = "my-msgs", produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<List<StateAndRef<MessageState>>> getMyMsgs() {
        List<StateAndRef<MessageState>> myMsgs = proxy.vaultQuery(MessageState.class).getStates().stream().filter(
                it -> it.getState().getData().getMsgSender().equals(proxy.nodeInfo().getLegalIdentities().get(0))).collect(Collectors.toList());
        return ResponseEntity.ok(myMsgs);
    }

    @GetMapping(value = "my-answers", produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<List<StateAndRef<VerificationState>>> getMyAnswers() {
        List<StateAndRef<VerificationState>> myAnswers = proxy.vaultQuery(VerificationState.class).getStates().stream().filter(
                it -> it.getState().getData().getSender().equals(proxy.nodeInfo().getLegalIdentities().get(0))).collect(Collectors.toList());
